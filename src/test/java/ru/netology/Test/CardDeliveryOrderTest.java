...
public class CardDeliveryOrderTest {
    ...
    @Test
    void shouldSuccessfulPlanMeeting() throws IOException {
        ...
        $(".button").click();
        String cityPoint = (String) $(".input__sub").text();
        if (cityPoint.equals("Доставка в выбранный город недоступна")) {
            $("[placeholder='Город']").sendKeys(Keys.chord(Keys.SHIFT, Keys.HOME), Keys.BACK_SPACE);
            $("[placeholder='Город']").setValue(DataGenerator.setNewCity());
            $(".button").click();
        }
    ...
    void shouldSuccessfulRescheduleMeeting() throws IOException {
        ...
        $("div.notification__content").shouldBe(visible,exactText("Встреча успешно запланирована на " + secondDayDelivery));
    }
}
